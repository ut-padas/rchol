...
/* functions set up */
void process_array(csc_form *input, std::vector<size_t> &result_idx, size_t depth, size_t target, std::vector<gsl_spmatrix *> &lap, size_t start, size_t total_size, int core_begin, int core_end); // read in matlab array to gsl sparray
...
void cholesky_factorization(std::vector<gsl_spmatrix *> &lap, csc_form *input, std::vector<size_t> &result_idx);
...
void rchol(csc_form *input, uint64_t *idx_data, uint64_t idxdim, int thread)
{
    NUM_THREAD = thread;
    cpu_set_t cpuset; 
...
void cholesky_factorization(std::vector<gsl_spmatrix *> &lap, csc_form *input,  std::vector<size_t> &result_idx)
{
    ...
    size_t *cpt = new size_t[m + 1]();
    size_t *rpt = new size_t[nzmax + 1]();
    double *datapt = new double[nzmax + 1]();
...
void process_array(csc_form *input, std::vector<size_t> &result_idx, size_t depth, size_t target, std::vector<gsl_spmatrix *> &lap, size_t start, size_t total_size, int core_begin, int core_end)
{
    int core_id = (core_begin + core_end) / 2;
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    ...
    {
        size_t i, j;
        size_t N = input->nsize;
        size_t *cpt = input->col;
        size_t *rpt = input->row;
        double *datapt = input->val;
